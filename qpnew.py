...
def update_ui():
    ...
    for i, title in enumerate(data["profiles"][active_profile]["titles"]):
        ...
        if edit_mode:
            ...
            up_btn = tk.Button(arrow_frame, text="▲", command=lambda i=i: move_entry(i, "up"), width=2, bg=button_bg, fg=fg_color)
            up_btn.pack(side="top")
            down_btn = tk.Button(arrow_frame, text="▼", command=lambda i=i: move_entry(i, "down"), width=2, bg=button_bg, fg=fg_color)
