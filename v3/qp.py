...
def save_window_position():
    if root:
        ...
        try:
            with open("window_config_tmp.json", "w") as temp_file:
                json.dump(config_data, temp_file)
            os.replace("window_config_tmp.json", "window_config.json")
...
def add_new_profile():
    ...
    # Aktivieren direkt nach Erstellen
...
def insert_text(index):
    try:
        ...
        print(f"⚠ Kein Text vorhanden für Hotkey-Index {index} im Profil '{active_profile}'")
        return
    pyperclip.copy(text)
    time.sleep(0.1)
...
def register_hotkeys():
    ...
    def hotkey_handler(index):
        ...
        if any(key in gedrückte_tasten for key in pfeiltasten):
            print("⚠ Windows-Funktion erkannt, kein Text eingefügt.")
            return  
        insert_text(index)
    ...
    for i, hotkey in enumerate(data["profiles"][active_profile]["hotkeys"]):
        if hotkey.strip():  
            ...
            if (
                len(keys) != 3
                or not keys[2]  # neuer Check: letzter Teil darf nicht leer sein
                or "ctrl" not in keys
            ...
            if i >= len(data["profiles"][active_profile]["texts"]):
                print(f"⚠ Hotkey '{hotkey}' zeigt auf Eintrag {i+1}, aber dieser existiert nicht im Profil '{active_profile}'")
                continue

            ref = keyboard.add_hotkey(hotkey, lambda index=i: hotkey_handler(index), suppress=False)
...
def quit_application(icon, item):
    try:
        ...
        print(f"⚠ Fehler beim Speichern der window_config.json: {e}")
...
def add_new_entry():
    ...
    # Nächsten verfügbaren Hotkey finden
    neuer_hotkey = ""
    for zeichen in erlaubte_zeichen:
        ...
        if test_hotkey not in belegte_hotkeys:
            ...
            break
    if not neuer_hotkey:
        neuer_hotkey = "ctrl+shift+"  # Fallback, falls alle belegt
...
def toggle_edit_mode():
    global edit_mode
    if edit_mode:  # Bearbeitungsmodus wird verlassen
        if has_unsaved_changes():
            result = messagebox.askyesno("Ungespeicherte Änderungen",
                                         "Du hast ungespeicherte Änderungen.\nWillst du sie speichern?")
            if result:  # Ja → speichern & Modus verlassen
                save_data()
                edit_mode = False
                update_ui()
                return
            else:  # Nein → bleib im Bearbeitungsmodus
                return
        else:
            edit_mode = False
            update_ui()
...
def save_data(stay_in_edit_mode=False):
    ...
    try:
        ...
        reset_unsaved_changes()  # ← VORHER hierhin verschieben
        update_ui()
...
def confirm_and_then(action_if_yes):
    # Wenn keine Änderungen, direkt ausführen
    if not has_unsaved_changes():
        action_if_yes()
        return

    # Wenn die Aktion das Speichern selbst ist → direkt ausführen
    if action_if_yes.__name__ == "save_data":
        action_if_yes()
        return

    # Sonst bestätigen und danach ausführen
    ...
    # Aktion leicht verzögert ausführen
...
def update_ui():
    ...
    # 💡 Nur wenn etwas ungespeichert ist → Meldung anzeigen
    if edit_mode and has_unsaved_changes():
        ...
        if result:
            save_data(stay_in_edit_mode=True)
            return  # 🔁 Wichtig: update_ui() wird dann erneut aufgerufen
        else:
            reset_unsaved_changes()
