...
class ComboBoxItemProxy:
    """Lightweight wrapper to access QComboBox items like QLineEdits."""
...
def update_ui():
    ...
    if profile_names:
        ...
        selector_layout.setSpacing(2 if app_state.mini_mode else 4)
        ...
        combo_height = scaled(26 if app_state.mini_mode else 34)
        combo.setFixedHeight(combo_height)
        combo.setMinimumWidth(scaled(140 if app_state.mini_mode else 210))
        radius = scaled(8 if app_state.mini_mode else 12)
        padding_v = scaled(4 if app_state.mini_mode else 6)
        padding_h = scaled(10 if app_state.mini_mode else 14)
        drop_width = scaled(22 if app_state.mini_mode else 26)
        ...
        if app_state.edit_mode:
            ...
            btn_size = scaled(26 if app_state.mini_mode else 30)
        ...
        def update_delete_state():
            ...
            if current_index < 0:
                ...
                return
            original = combo.itemData(current_index)
        ...
        combo.currentIndexChanged.connect(on_profile_index_changed)
